...
from .bert_model import BertSelfAttnLayer


class MergeAttentionEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.cross_modal_layers = nn.ModuleList([BertSelfAttnLayer(bert_config, config) for _ in range(config.num_cross_encoder_layers)])
    ...
    def forward(self, attention_mask, text_feat=None, visual_feat=None, audio_feat=None):
        ...
        for cross_modal_layer in self.cross_modal_layers:
            z = cross_modal_layer(z, extended_attention_mask)
            z = z[0]

...
class Seqeval(datasets.Metric):
    ...
    def compute_metric_example_level(self, predictions_logits, labels, label_list, custom_args, data_args, reverse_logits=False, ts_score_predictor="lt", mode="test"):
        if ts_score_predictor == "lt":
            # from lt
            ...
            # from cos
            predictions = [(np.array(example_logits) > 0.5).astype(np.int32).tolist() for example_logits in predictions_logits]
            seg_point_predictions_scores = [[1 - v for v in example_logits] for example_logits in predictions_logits]
        if reverse_logits:
        ...
        # Remove ignored index (special tokens). here is redundant because predictions and labels is eop level
        true_predictions = [

...
def main():
    ...
    if training_args.do_train or data_args.infer_train_data:
    ...
    if "schen" in model_args.text_encoder_name_or_path:
        from models.text_encoder.longformer_schen.longformer import LongformerConfig
        config = LongformerConfig.from_pretrained(model_args.text_encoder_name_or_path) 
    else:
    ...
    if "longformer" in model_args.text_encoder_name_or_path:
        print("running LongformerTokenizerFast.from_pretrained")
    ...
    if "longformer".lower() in model_args.text_encoder_name_or_path.lower() or "bert" in model_args.text_encoder_name_or_path.lower():
        from models.multi_modal_for_ts import MultiModalForTS
        
        if model_args.init_model:
            model = MultiModalForTS(config)
        else:
            model = MultiModalForTS.from_pretrained(
    ...
    def prepare_features_with_dynamic_num_sentence(examples):
        ...
        for example_index in tqdm(range(num_examples)):
            ...
            # vis features related
            vis_2d_feature_dict = get_example_cached_feature_paths(data_args.vis2d_feature_cache_dir, lecture)
            vis_3d_feature_dict = get_example_cached_feature_paths(data_args.vis3d_feature_cache_dir, lecture)
            vis_ocr_feature_dict = get_example_cached_feature_paths(data_args.vis_ocr_feature_cache_dir, lecture)
            audio_feature_dict = get_example_cached_feature_paths(data_args.audio_feature_cache_dir, lecture)
            ...
            while sent_i < len(accumulate_length):
                ...
                if token_right_index - token_left_index >= max_seq_length - 1 or token_right_index == example_total_num_tokens:
                    ...
                    # vis features related
                    def load_cached_feature(feat_type, lecture, feature_path_dict, sent_left_index, sent_right_index, hidden_size):
                        res = []
                        for clip_index in range(sent_left_index, sent_right_index):
                            if clip_index in feature_path_dict:
                                clip_emb = np.load(feature_path_dict[clip_index])
                                if len(clip_emb.shape) == 2 and clip_emb.shape[0] != 0:
                                    print("len(clip_emb.shape) == 2 and clip_emb.shape[0] != 0")
                                    clip_emb = np.max(clip_emb, axis=0).tolist()
                                if clip_emb.shape[0] == 0:
                                    print("clip_emb.shape[0] == 0")
                                    clip_emb = [0.0] * hidden_size
                            else:
                                print("{} clip_index {} not in lecture {} 's feature_path_dict.keys() {}".format(feat_type, clip_index, lecture, feature_path_dict.keys()))
                                clip_emb = [0.0] * hidden_size
                            res.append(clip_emb)
                    ...
                    if not config.use_raw_shot:
                        cur_vis_2d_embeds = load_cached_feature("2d", lecture, vis_2d_feature_dict, sent_left_index, sent_right_index, model_args.hidden_size_vis2d)
                        new_vis2d_embeds.append(cur_vis_2d_embeds)

                        cur_vis_3d_embeds = load_cached_feature("3d", lecture, vis_3d_feature_dict, sent_left_index, sent_right_index, model_args.hidden_size_vis3d)
                        new_vis3d_embeds.append(cur_vis_3d_embeds)

                        cur_vis_ocr_embeds = load_cached_feature("ocr", lecture, vis_ocr_feature_dict, sent_left_index, sent_right_index, model_args.hidden_size_vis_ocr) 
                        new_vis_ocr_embeds.append(cur_vis_ocr_embeds)
                    
                    cur_audio_embeds = load_cached_feature("audio",lecture, audio_feature_dict, sent_left_index, sent_right_index, model_args.hidden_size_audio) 
    ...
    def compute_metrics(p):
        ...
        if config.out_modal_prob:
            print("out_modal_prob is True")
            print("predictions_logits.shape: ", predictions_logits.shape)
            mm, text_logits, vis_logits = np.split(predictions_logits, 3, axis=2)
            predictions_logits = mm
    ...
    optimizer = optim.Adam([
        ...
        cross_encoder_params,   # 特定模块的自定义学习率
    ])
    ...
    # Predict
    if training_args.do_predict:
        if data_args.infer_train_data:
            metric_key_prefix = "infer_train"
            logger.info("*** Predict train set ***")
            sentences = train_dataset["sentences"]
            example_ids = train_dataset["example_ids"]
            num_examples = len(train_examples[context_column_name])
            num_samples = len(train_dataset["sentences"])
            print("num predict samples: ", num_samples)
            print("num predict examples: ", num_examples)

            start_time = time.time()
            predict_logits, labels, metrics = trainer.predict(train_dataset, metric_key_prefix=metric_key_prefix)
            end_time = time.time()
        ...
        text_logits, vis_logits = None, None
        if config.out_modal_prob:
            print("out_modal_prob is True")
            mm, text_logits, vis_logits = np.split(predict_logits, 3, axis=2)
            predict_logits = mm
        ...
        if config.out_modal_prob:
            text_predictions = np.argmax(text_logits, axis=2)
            vis_predictions = np.argmax(vis_logits, axis=2)
            true_text_predictions = [
                [label_list[prediction[i]] for i in range(len(label))]  # prediction[:k] 是有效的logits，k是label（一个seq样本的标签）中非-100的数目
                for prediction, label in zip(text_predictions, true_labels)
            ]
            true_text_predict_logits = [
                [p_logits[i].tolist() for i in range(len(label))]
                for p_logits, label in zip(text_logits, true_labels)
            ]

            true_vis_predictions = [
                [label_list[prediction[i]] for i in range(len(label))]  # prediction[:k] 是有效的logits，k是label（一个seq样本的标签）中非-100的数目
                for prediction, label in zip(vis_predictions, true_labels)
            ]
            true_vis_predict_logits = [
                [p_logits[i].tolist() for i in range(len(label))]
                for p_logits, label in zip(vis_logits, true_labels)
            ]
        ...
        out = [{"example_id": [], "sentences": [], "labels": [], "int_labels": [], "predictions": [], "predict_logits": [], "text_predictions": [], "text_logits": [], "vis_predictions": [], "vis_logits": []} for _ in range(num_examples)]
        if trainer.is_world_process_zero():
            with open(output_predictions_file, "w") as writer:
                if config.out_modal_prob:
                    for prediction, sentence_list, label, int_labels, example_id, predict_logits, text_pred, text_logit, vis_pred, vis_logit in zip(
                        true_predictions, sentences, true_labels, true_int_labels, example_ids, true_predict_logits, true_text_predictions, true_text_predict_logits, true_vis_predictions, true_vis_predict_logits):
                        ...
                        out[example_id]["text_predictions"].extend(text_pred)
                        out[example_id]["text_logits"].extend(text_logit)
                        out[example_id]["vis_predictions"].extend(vis_pred)
                        out[example_id]["vis_logits"].extend(vis_logit)
                else:

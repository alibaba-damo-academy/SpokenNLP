...
class BertSelfAttention(nn.Module):
    def __init__(self, config, ce_kv_hidden_size=None):
        super().__init__()
        ...
        if ce_kv_hidden_size is not None:
            self.key = nn.Linear(ce_kv_hidden_size, self.all_head_size)
            self.value = nn.Linear(ce_kv_hidden_size, self.all_head_size)
        else:
    ...
    def forward(
        ...
        # print("attention_probs.shape: {}, value_layer.shape: {}".format(attention_probs.shape, value_layer.shape))
...
class BertAttention(nn.Module):
    def __init__(self, config, ce_kv_hidden_size=None):
        super().__init__()
        self.self = BertSelfAttention(config, ce_kv_hidden_size=ce_kv_hidden_size)
...
class BertCrossLayer(nn.Module):
    def __init__(self, config, ce_kv_hidden_size=None):
        ...
        self.crossattention = BertAttention(config, ce_kv_hidden_size=ce_kv_hidden_size)
...
class BertMoELayer(nn.Module):
    ...
    def forward(
        ...
        batch_size, seq_length, hidden_size = attention_output.shape
        attention_output = attention_output.view(batch_size * seq_length, hidden_size)

        layer_output, moe_loss = self.moe(attention_output)
        layer_output = layer_output.view(batch_size, seq_length, hidden_size)
        
        outputs = self_attention_outputs[1:]  # add self attentions if we output attention weights
        outputs = (layer_output, moe_loss) + outputs

...
def get_feat_iterator(feat_dir, split, nshard, rank, down_sampling):
    ...
    def iterate():
        ...
        for offset, leng in zip(offsets, lengs):
            sampled_feat = feat[offset: offset + leng]
            if down_sampling > 1:
...
def dump_label(feat_dir, split, km_path, nshard, rank, lab_dir, down_sampling):
    apply_kmeans = ApplyKmeans(km_path)
    generator, num = get_feat_iterator(feat_dir, split, nshard, rank, down_sampling)
    iterator = generator()
...
if __name__ == "__main__":
    ...
    parser.add_argument("down_sampling", type=int)

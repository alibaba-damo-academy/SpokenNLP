...
from transformers.models.longformer.modeling_longformer import LongformerPreTrainedModel, LongformerModel, LongformerSequenceClassifierOutput, LongformerClassificationHead
...
class LongformerWithDAForSentenceLabelingTopicSegmentation(LongformerPreTrainedModel):
    ...
    def forward(
        ...
        if global_attention_mask is None:
            global_attention_mask = torch.zeros_like(input_ids[:, 0])
            # global attention on cls token
            global_attention_mask[:, 0] = 1
            
        anchor_outputs = self.longformer(
            input_ids[:, 0],

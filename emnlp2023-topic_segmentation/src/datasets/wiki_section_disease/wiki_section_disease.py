...
class WikiSectionDiseaseForDocumentSegmentation(datasets.GeneratorBasedBuilder):
    BUILDER_CONFIGS = [
        DocumentSegmentationConfig(name="WikiSectionDiseaseForDocumentSegmentation", version=version, description=desc),
    ]
    ...
    def _generate_examples(self, filepath):
        ...
        with open(filepath, "r") as f:
            for example_id, line in enumerate(f.readlines()):
                ...
                labels = [label_map[v] if v in label_map else -100 for v in example["labels"]]
